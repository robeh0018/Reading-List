...
export class BookService {
    ...
    switchIsReadingBook(id: string): void {
        ...
        if (bookIndex !== -1) {
            ...
            this.toastService.show(
                `${this.handleToastSuccessMessage(updatedBookList[bookIndex])}`,
                {classname: 'bg-success text-light', delay: 4000, header: 'Successfully!!'});
        }
    };
    ...
    private handleToastSuccessMessage(book: IBook): string {
        const bookTitle = book.title.substring(0, 15);

        return !book.isReading
            ? `(${bookTitle} ...) Removed From the Reading List`
            : `(${bookTitle} ...) Added To the Reading List`
    }
}
